...
class MainWindow(Qt.QMainWindow):
    ...
    def initUI(self, cfg):
        ...
        col1_widget.setFixedWidth(350)
        col3_widget.setFixedWidth(200)
        ...
        self.canvas = MplCanvas(parent=self, width=20, height=20, dpi=100)
    ...
    def output_error(self, error):
        self.output_to_info(f"<span style=\" font-weight:500; color:#ff0000;\" >{error}</span>")

    def output_to_info(self, text, elapsed_time=None):
        if elapsed_time is not None:
            self.info_box.append(f"{text}. Время исполнения: {round(elapsed_time, 3)}мс")
        else:
